...
class GoodsLogic extends Model
{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $list_brand = Db('brand')->where("id in(select brand_id from " .config('database.prefix') . "goods where brand_id > 0 and goods_id in(:goods_id_str))")->bind(['goods_id_str'=>$goods_id_str])->limit('30')->select();  //where("goods_id in($goods_id_str)")->select();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $spec =Db('spec')->where(array('search_index'=>1))->Field('id,name')->select();
        $spec_item =Db('spec_item')->where(array('spec_id'=>array('in',array_keys($spec))))->Field('id,spec_id,item')->select();
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $goods_attr =Db('goods_attr')->where(['goods_id'=>['in',$goods_id_str],'attr_value'=>['<>','']])->select();
        // $goods_attr =Db('goods_attr')->where("attr_value != ''")->select();
        $goods_attribute =Db::name('goods_attribute')->Field('attr_id,attr_name,attr_index')->select();
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
        ...
        foreach ($goods_attr as $k => $v) {
            ...
            $list_attr[$v['attr_id']]['attr_value'][] = array('key' => $v['attr_id'], 'val' => $v['attr_value'], 'attr_value' => $v['attr_value'], 'href' => urldecode(url("Goods/$action", $filter_param, '')));
    ...
    {
        $c1 =Db('Comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(4,5)")->bind(['goods_id'=>$goods_id])->count(); // 好评
        $c2 =Db('Comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(3)")->bind(['goods_id'=>$goods_id])->count(); // 中评
        $c3 =Db('Comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(1,2)")->bind(['goods_id'=>$goods_id])->count(); // 差评
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        //$count =Db('Goods')->where("goods_id = $goods_id")->count();
        //if($count == 0)  return array('status'=>-2,'msg'=>'收藏的商品不存在','result'=>array());
        $count =Db('GoodsCollect')->where("user_id",$user_id)->where("goods_id", $goods_id)->count();
        if ($count > 0) return array('status' => -3, 'msg' => '商品已收藏', 'result' => array());
       Db('GoodsCollect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()));
    ...
    public function get_spec($goods_id)
    {
        ...
        $keys =Db('SpecGoodsPrice')->where("goods_id", $goods_id)->Field("GROUP_CONCAT(`key` SEPARATOR '_') ")->select();
        $filter_spec = array();
        if ($keys) {
            $specImage =Db('SpecImage')->where(['goods_id'=>$goods_id,'src'=>['<>','']])->Field("spec_image_id,src")->select();// 规格对应的 图片表， 例如颜色
    ...
    public function get_siblings_cate($cat_id)
    {
        if (empty($cat_id))
            return array();
        $cate_info =Db('goods_category')->where("id", $cat_id)->find();
        $siblings_cate =Db('goods_category')->where(['id'=>['<>',$cat_id],'parent_id'=>$cate_info['parent_id']])->select();
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $priceList =Db('goods')->where("goods_id", "in", $goods_id_str)->Field('shop_price', true)->select();  //where("goods_id in($goods_id_str)")->select();
        rsort($priceList);
    ...
    function get_filter_menu($filter_param, $action)
    {
        ...
        if (!empty($filter_param['spec'])) {
            $spec =Db::name('spec')->Field('id,name')->select();
            $spec_item =Db::name('spec_item')->Field('id,item')->select();
        ...
        if (!empty($filter_param['attr'])) {
            $goods_attribute =Db::name('goods_attribute')->Field('attr_id,attr_name')->select();
    ...
    function get_goods_cate(&$goodsCate)
    {
        ...
        if ($goodsCate['level'] == 1) {
            ...
            $parent =Db('GoodsCategory')->where("id", $goodsCate['parent_id'])->order('`sort_order` desc')->find();//父类
    ...
    function getGoodsIdByBrandPrice($brand_id, $price)
    {
        ...
        $arr =Db('goods')->where($where)->bind($bind)->Field('goods_id', true)->select();
    ...
    function getGoodsIdByAttr($attr)
    {
        ...
        $c = count($attr_id) - 1;
        if ($c > 0)
            $arr =Db('goods_attr')->where($where)->bind($bind)->group('goods_id')->having("count(goods_id) > $c")->Field("goods_id", true)->select();  //select * from   `tp_goods_attr` where attr_id in(59,80) and attr_value in('直板','翻盖','蓝牙4.0') group by goods_id having count(goods_id) > 1
        else
            $arr =Db('goods_attr')->where($where)->bind($bind)->Field("goods_id", true)->select(); // 如果只有一个条件不再进行分组查询
    ...
    {
        $parent_region =Db('region')->where(array('level'=>1))->field('id,name')->cache(true)->select();
        $ip_location = array();
        $city_location = array();
        foreach($parent_region as $key=>$val){
            $c =Db('region')->where(array('parent_id'=>$parent_region[$key]['id']))->field('id,name')->order('id asc')->cache(true)->select();
    ...
    function getParentRegionList($cid){
        //$pids = '';
        $pids = array();
        $parent_id = Db('region')->where(array('id'=>$cid))->Field('parent_id')->select();
        if($parent_id != 0){
            //$pids .= $parent_id;
    ...
    function getGoodsDispatching($goods_id,$region_id)
    {
        ...
        $goods =Db('goods')->where(array('goods_id'=>$goods_id))->find();
        //检查商品是否包邮
        ...
        if(empty($goods['shipping_area_ids'])){
            $plugin_goods_shipping =Db('plugin')->where(array('type'=>'shipping'))->select();
        ...
        $find_shipping_area_id =Db('area_region')->where(array('region_id'=>array('in',$parent_region_id)))->group('shipping_area_id')->Field('shipping_area_id',true)->select();
        ...
        if(count($shipping_area_id_array) == 0){
            $goods_shipping =Db('shipping_area')->where(array('shipping_area_id'=>array('in',$goods_shipping_area_id_array),'is_default'=>1))->select();
            ...
            if(!empty($goods_shipping)){
                foreach($goods_shipping as $k=>$v){
                    ...
                    $goods_shipping[$k]['shipping_name'] =Db('plugin')->where(array('type'=>'shipping','code'=>$goods_shipping[$k]['shipping_code']))->Field('name')->select();
                }
                $return_data['msg'] = '有货';
                $return_data['result'] = $goods_shipping;
                return $return_data;
    ...
    function get_filter_selected($sel ,$cat_id = 1){
        $where = " 1 = 1 ";
        $Goods =Db('goods')->where("cat_id" ,"in" ,implode(',', $cat_id));
        //查看全部
        if($sel == 'selall'){
            $where .= '';
        }
        ...
        //看全部
        if($sel == 'all'){
            $arrid = $Goods->Field('goods_id', true)->select();
        }else{
            $arrid = $Goods->where($where)->Field('goods_id', true)->select();
        }
        return $arrid;
    }
}